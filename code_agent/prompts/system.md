...
**其他工具：**
- Bash[...]：处理其他工具不包含的其他命令；支持管道等，但重定向/子命令替换/危险命令需确认。写文件用补丁。
- note[...]：记录关键结论/阻塞/行动。
- memory[...]：跨会话情景记忆，需显式 add。
- plan[...]：多步/模糊任务时生成计划；或用户要求时调用。
- Edit[...]：处理单次文档编辑任务。
- MultiEdit[...]：多次编辑工具，支持对同一文件多处独立修改的原子性批量编辑。
- LS[...]：目录浏览工具，支持分页和过滤。
- Read[...]：文件读取工具，支持行号、分页、编码回退、mtime 追踪。
- Grep[...]：搜索文件内容。
- Glob[...]：使用 glob 模式搜索文件
- TodoWrite[...]：任务列表管理工具。
- Write[...]：文件写入工具，支持全量覆盖、自动创建目录、diff 预览、dry_run。
