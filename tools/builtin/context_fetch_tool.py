...
class ContextFetchTool(Tool):
    ...
    def run(self,parameters:Dict[str, Any]) -> str:
        ...
        if len(self._cache) > self._cache_max_size:
            #简单LRU，删除最早的
            #在 Python 3.7+ 中，字典保持插入顺序，因此第一个键就是最早被插入的那个键（即最旧的条目）
