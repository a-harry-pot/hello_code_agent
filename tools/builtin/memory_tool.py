...
class MemoryTool(Tool):
    ...
    def _get_summary(self,limit:int =10)->str:
        ...
        try:
            ...
            if important_memories:
                ...
                #按重要性排序
                unique_memories.sort(key=lambda memory: memory.importance, reverse=True)
    ...
    def _forget(self,strategy:str="importance_based",threshold:float=0.1,max_age_days:int=30)->str:
        """遗忘记忆（支持多种策略）"""
        try:
            count=self.memory_manager.retrieve_memories(
                strategy=strategy,
                threshold=threshold,
    ...
    def clear_session(self):
        ...
        # 清理工作记忆
        wm=self.memory_manager.memory_type.get("working") if hasattr(self.memory_manager, "memory_type") else None
        if wm:
            wm.clear()
