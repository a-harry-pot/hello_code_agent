...
class WorkingMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 工作记忆特定配置
        self.max_capacity = self.config.working.max_capacity
        self.max_tokens = self.config.working.max_tokens
    ...
    def retrieve(self, query: str, limit: int = 5, user_id: str = None, **kwargs) -> List[MemoryItem]:
        ...
        for memory in filtered_memories:
            ...
            # 时间衰减
            time_decay = self._caculate_time_decay(memory.timestamp)
    ...
    def forget(self,strategy:str="importance_based",threshold:float=0.1,max_age_days:int=1)->int:
        ...
        if strategy == "importance_based":
            ...
            if len(self.memories)>self.max_capacity:
                #按照优先级顺序，删除最低的
                sorted_memories = sorted(self.memories, key=lambda x: self._caculate_priority(x))
