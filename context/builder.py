...
class ContextBuilder:
    ...
    def build_base(
            self,
            user_query: str,
            conversation_history: Optional[Message] = None,
        ...
        if conversation_history:
            recent_history = conversation_history[-self.config.max_history_turns:]  # 负值，倒着取
            history_text = "\n".join([
        ...
        # P2:上次工具调用摘要（如有）
        if tool_summaries:
            summary_text = "\n".join(tool_summaries[-3:])  # 最多保留最近3条
            packets.append(ContextPacket(
                content=f"[上次工具结果摘要]{summary_text}",
    ...
    def _structure_base(
        ...
        # [Role & Policies]
        instructions = [p for p in packets if p.metadata["type"] == "instructions"]
        if instructions:
    ...
    def _gather(
        ...
        当lazy_fetch=True时，只收集保底上下文（系统指令+对话历史+额外包）
        ...
        #以下为扩展上下文，仅在lazy_fetch=False时主动收集
    ...
    def _select(
        ...
        for packet in packets:
            ...
            if len(query_tokens)>0:
                overlap=len(query_tokens & content_tokens) # 看两个词set的交集
        ...
        # 4)系统指令与对话历史单独拿出，固定纳入（基础上下文不应该被过滤）
        ...
        # 5)根据min_relevance过滤（仅对扩展上下文：memory,RAG,notes等）
    ...
    def _compress(self,context:str)->str:
        ...
        # LLM压缩（更保真）：仅在提供llm时启用
        if self.llm is not None:
            try:
                target=available_tokens
                #尽量保留结构：Role/Task 原样，压缩Evidence/Context
                sys=(
